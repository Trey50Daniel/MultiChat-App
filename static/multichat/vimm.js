if(vimmname){
  ...
  Vimm Functions
  */
  function connectVimm() {
    var chatSocketVimm = new ReconnectingWebSocket('wss://www.vimm.tv:9001/ws/chat/'+vimmname+'/');
  
    //This gets executed when the client makes a successful connection to the chat server
    chatSocketVimm.onopen = function(e) {
    ...
    chatSocketVimm.onclose = function(e) {
    ...
    chatSocketVimm.onerror = function(e) {
      console.error('An error caused the chat client to disconnect.');
  
      //close the connection to trigger the auto reconnection
      chatSocketVimm.close();
    }
  
    //This catches messages from the websocket and interacts with the DOM according the the type of message
    chatSocketVimm.onmessage = function(e) {
      var data = JSON.parse(e.data);
      var message = data['message'];
      var chatter = data['chatter'];
      var prefix = data['prefix'];
      var type = data['mtype'];
      var prefix2 = prefix.replace('[dev]','<span class="badge badge-primary badge-square" title="Vimm.tv Developer"><i class="fa fa-cog"></i></span>').replace('[op]','<span class="badge badge-primary badge-square" title="Vimm.tv Admin"><i class="fas fa-gavel"></i></span>').replace('[rep]','<span class="badge badge-primary badge-square" title="Vimm.tv Curator"><i class="fa fa-chevron-circle-up"></i></span>').replace('[sup]','<span class="badge badge-warning badge-square" title="Vimm.tv Supporter"><i class="fas fa-hands-helping"></i></span>');
  
      if(type == "message"){
        if (data['chatter'] == "vimm"){
    ...
    function keepAlive(){
      chatSocketVimm.send(JSON.stringify({
        'mtype': 'signal',
        'message': '',
        'chatter': '{{room_name}}',
        'channel': '{{room_name}}',
                  ...
                  if(tokenHistory[i].operation == "tokens_transfer" && tokenHistory[i].from != steem_id){
                      ...
                      if(!window.txids.includes(tokenHistory[i].transactionId)){
                          ...
                          if(chatNotif){
                              ...
                              showMessageVimm(vimmname,'chat',"VimmBot",gemMsgString,fade);
  ...
  function showMessageVimm(chan, type, chatter, message, timeout = 0) {
    ...
    if(type === 'chat') {
      ...
      nameEle.classList.add('user-name-vimm');
      ...
      if(discordwebhook){
        sendToDiscord(chatter, "Vimm", 'https://www.vimm.tv/@'+chatter+'/avatar', HTMLDecode(message.replace(/(<([^>]+)>)/ig,"")));
      }
    }
    else if(type === 'admin') {
